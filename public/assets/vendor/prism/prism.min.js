                ...
                for (var v = r, k = i; v < t.length; k += t[v].length, ++v) {
                  ...
                  if (!(P instanceof o)) {
                    ...
                    if (!w && h && v != t.length - 1) {
                      ...
                      for (
                        var S = w.index + (d ? w[1].length : 0),
                          A = w.index + w[0].length,
                          j = v,
                          _ = k,
                          C = t.length;
                        j < C && (_ < A || (!t[j].type && !t[j - 1].greedy));
                        ++j
                      )
                        S >= (_ += t[j].length) && (++v, (k = _));
                      if (t[v] instanceof o || t[j - 1].greedy) continue;
                      (x = j - v), (P = e.slice(k, _)), (w.index -= k);
                    }
                    if (w) {
                      d && (f = w[1].length);
                      var A = (S = w.index + f) + (w = w[0].slice(f)).length,
                        N = P.slice(0, S),
                        O = P.slice(A),
                        E = [v, x];
                      N && (++v, (k += N.length), E.push(N));
                      var F = new o(u, m ? a.tokenize(w, m) : w, y, w, h);
                      if (
                        (E.push(F),
                        O && E.push(O),
